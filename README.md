...
2. **Watch the metrics update**:
   ...
   - **In-stock probability** $\mathbb{Pr}(D \leq q)$
...
3. **Interpret the charts** on the right:
   ...
   - **Demand density plot (bottom left)**: shows the Normal demand curve and shades the area $\mathbb{Pr}(D \leq q)$, illustrating the in-stock probability at the current \(q\).
